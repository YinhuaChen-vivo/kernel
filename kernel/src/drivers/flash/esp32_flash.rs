...
// Returns the start of the linker-reserved SRAM window for XIP ELF writable
// segments. Its size is part of the board's loader configuration.
...
impl Esp32FlashDevice {
    ...
    /// Write __loader_rw_start (reserved SRAM base for RW segments) to the caller's
    ...
    fn ioctl_query_dram_safe(&self, arg: usize) -> Result<(), ErrorKind> {
        if arg != 0 {
            let safe = core::ptr::addr_of!(crate::boot::__loader_rw_start) as u32;
...
mod tests {
    ...
    fn region_absolute_offset_basic() {
        ...
        assert_eq!(
            region().absolute_offset(LOADABLE_REGION_SIZE, 0),
            Ok(0x0020_0000)
        );
    }
